// src/data/productInfo.js

export const PRODUCT_INFO = {



실속형PB국산: {
    manufacturer: "케미",
    desc: "가볍게 사용하는 실속형",
    grade: "합리적 선택",
    badge: "입문 추천",
    variants: {


C0: {
        summary: "부담 없는 실속누진.",
        goodFor: ["가벼운 근거리 작업", "가격 부담이 큰 고객"],
        points: ["실속형", "부담 적은 선택", "기본 기능 중심"],
        image: "/images/ASIO.jpg",
        url: null,

        regularPrices: {
          "1.50": "14만원",
          "1.60": "19.8만원",
          "1.67": "23.8만원",
          "1.74": "29.8만원",
        },
        salePrices: {
          "1.50": "5.9만원",
          "1.60": "7.9만원",
          "1.67": "10.9만원",
          "1.74": "13.9만원",
        },

        tintRegularPrices: {
          "1.50": "3만원",
          "1.60": "3만원",
          "1.67": "3만원",
          "1.74": "3만원",
        },
        tintSalePrices: {
          "1.50": "1.5만원",
          "1.60": "1.5만원",
          "1.67": "1.5만원",
          "1.74": "1.5만원",
        },

        photoRegularPrices: {
          "1.50": "15만원",			
          "1.60": "10만원",
          "1.67": "10만원",
        },
        photoSalePrices: {
		  "1.50": "7만원",
          "1.60": "5만원",
          "1.67": "5만원",
        },

        premiumCoatingRegularPrices: {
          "1.50": "3만원",
          "1.60": "3만원",
          "1.67": "3만원",
          "1.74": "3만원",
        },
        premiumCoatingSalePrices: {
          "1.50": "1만원",
          "1.60": "1만원",
          "1.67": "1만원",
          "1.74": "1만원",
        },
      },
	  },
	  },


기본형PB국산: {
    manufacturer: "케미",
    desc: "가볍게 사용하는 기본형",
    grade: "합리적 선택",
    badge: "입문 추천",
    variants: {


C1: {
        summary: "부담 없는 기본형누진.",
        goodFor: ["가벼운 근거리 작업", "가격 부담이 큰 고객", "입문형 누진다초점"],
        points: ["기본형", "부담 적은 선택", "기본 기능 중심"],
        image: "/images/C1.jpg",
        url: null,

        regularPrices: {
          "1.50": "19.8만원",
          "1.60": "23.8만원",
          "1.67": "29.8만원",
          "1.74": "43.8만원",
        },
        salePrices: {
          "1.50": "8.9만원",
          "1.60": "11.9만원",
          "1.67": "14.9만원",
          "1.74": "20.9만원",
        },

        tintRegularPrices: {
          "1.50": "3만원",
          "1.60": "3만원",
          "1.67": "3만원",
          "1.74": "3만원",
        },
        tintSalePrices: {
          "1.50": "1.5만원",
          "1.60": "1.5만원",
          "1.67": "1.5만원",
          "1.74": "1.5만원",
        },

        photoRegularPrices: {
          "1.50": "15만원",			
          "1.60": "10만원",
          "1.67": "10만원",
        },
        photoSalePrices: {
		  "1.50": "8만원",
          "1.60": "5만원",
          "1.67": "5만원",
        },

        premiumCoatingRegularPrices: {
          "1.50": "3만원",
          "1.60": "3만원",
          "1.67": "3만원",
          "1.74": "3만원",
        },
        premiumCoatingSalePrices: {
          "1.50": "1만원",
          "1.60": "1만원",
          "1.67": "1만원",
          "1.74": "1만원",
        },
      },
	  },
	  },

고급형PB국산: {
    manufacturer: "케미",
    desc: "가볍게 사용하는 고급형누진",
    grade: "합리적 선택",
    badge: "입문 추천",
    variants: {


      C4: {
        summary: "부담 없는 실속노멀버전.",
        goodFor: ["가벼운 근거리 작업", "가격 부담이 큰 고객", "입문형 누진다초점"],
        points: ["실속형", "부담 적은 선택", "기본 기능 중심"],
        image: "/images/C3.jpg",
        url: null,

        regularPrices: {
          "1.50": "32.5만원",
          "1.60": "38만원",
          "1.67": "44.5만원",
          "1.74": "59만원",
        },
        salePrices: {
          "1.50": "13.9만원",
          "1.60": "16.9만원",
          "1.67": "20.9만원",
          "1.74": "25.9만원",
        },

        tintRegularPrices: {
          "1.50": "3만원",
          "1.60": "3만원",
          "1.67": "3만원",
          "1.74": "3만원",
        },
        tintSalePrices: {
          "1.50": "1.5만원",
          "1.60": "1.5만원",
          "1.67": "1.5만원",
          "1.74": "1.5만원",
        },

        photoRegularPrices: {
          "1.50": "15만원",			
          "1.60": "10만원",
          "1.67": "10만원",
        },
        photoSalePrices: {
		  "1.50": "8만원",
          "1.60": "5만원",
          "1.67": "5만원",
        },

        premiumCoatingRegularPrices: {
          "1.50": "3만원",
          "1.60": "3만원",
          "1.67": "3만원",
          "1.74": "3만원",
        },
        premiumCoatingSalePrices: {
          "1.50": "1만원",
          "1.60": "1만원",
          "1.67": "1만원",
          "1.74": "1만원",
        },
      },
	  },
	  },


기본형PB수입: {
    manufacturer: "에실로",
    desc: "가성비 좋은 수입PB다초점",
    grade: "합리적 선택",
    badge: "입문 추천",
    variants: {



      E2: {
        summary: "디지털 디바이스 특화설계로 조금더 편안하게.",
        goodFor: ["디지털기기 특화", "근거리 시야 무난", "가성비좋은 제품"],
        points: ["실내 중심", "근용 특화", "합리적인 가격"],
        image: "/images/E22.jpg",
        url: null,

        regularPrices: {
          "1.50": "39만원",
          "1.60": "47만원",
          "1.67": "55만원",
        },
        salePrices: {
          "1.50": "19.5만원",
          "1.60": "23.5만원",
          "1.67": "27.5만원",
        },

        tintRegularPrices: {
          "1.50": "3만원",
          "1.60": "3만원",
          "1.67": "3만원",
        },
        tintSalePrices: {
          "1.50": "1.5만원",
          "1.60": "1.5만원",
          "1.67": "1.5만원",
        },

        photoRegularPrices: {
          "1.50": "20만원",
          "1.60": "20만원",
          "1.67": "20만원",
        },
        photoSalePrices: {
          "1.50": "10만원",
          "1.60": "10만원",
          "1.67": "10만원",
        },

        premiumCoatingRegularPrices: {
          "1.60": "5만원",
          "1.67": "5만원",
        },
        premiumCoatingSalePrices: {
          "1.60": "3만원",
          "1.67": "3만원",
        },
      },
	  },
	  },







    국산고급형: {
    manufacturer: "케미",
    desc: "가볍게 사용하는 기본누진",
    grade: "합리적 선택",
    badge: "입문 추천",
    variants: {
      C6: {
        summary:
          "누진다초점 적응이 수월하고 사용량이 많지 않은 고객에게 부담 없이 권하기 좋은 제품입니다.",
        goodFor: ["가벼운 근거리 작업", "가격 부담이 큰 고객", "입문형 누진다초점"],
        points: ["실속형", "부담 적은 선택", "기본 기능 중심"],
        image: "/images/C6.jpg",
        url: null,

        regularPrices: {
          "1.50": "36.5만원",
          "1.60": "43.5만원",
          "1.67": "51만원",
          "1.74": "64만원",
        },
        salePrices: {
          "1.50": "17.9만원",
          "1.60": "20.9만원",
          "1.67": "27.9만원",
          "1.74": "39.9만원",
        },

        tintRegularPrices: {
          "1.50": "3만원",
          "1.60": "3만원",
          "1.67": "3만원",
          "1.74": "3만원",
        },
        tintSalePrices: {
          "1.50": "1.5만원",
          "1.60": "1.5만원",
          "1.67": "1.5만원",
          "1.74": "1.5만원",
        },

        photoRegularPrices: {
          "1.50": "15만원",			
          "1.60": "10만원",
          "1.67": "10만원",
        },
        photoSalePrices: {
		  "1.50": "8만원",
          "1.60": "5만원",
          "1.67": "5만원",
        },

        premiumCoatingRegularPrices: {
          "1.50": "3만원",
          "1.60": "3만원",
          "1.67": "3만원",
          "1.74": "3만원",
        },
        premiumCoatingSalePrices: {
          "1.50": "1만원",
          "1.60": "1만원",
          "1.67": "1만원",
          "1.74": "1만원",
        },
      },

      C4: {
        summary: "부담 없는 실속노멀버전.",
        goodFor: ["가벼운 근거리 작업", "가격 부담이 큰 고객", "입문형 누진다초점"],
        points: ["실속형", "부담 적은 선택", "기본 기능 중심"],
        image: "/images/C3.jpg",
        url: null,

        regularPrices: {
          "1.50": "32.5만원",
          "1.60": "38만원",
          "1.67": "44.5만원",
          "1.74": "59만원",
        },
        salePrices: {
          "1.50": "13.9만원",
          "1.60": "16.9만원",
          "1.67": "20.9만원",
          "1.74": "25.9만원",
        },

        tintRegularPrices: {
          "1.50": "3만원",
          "1.60": "3만원",
          "1.67": "3만원",
          "1.74": "3만원",
        },
        tintSalePrices: {
          "1.50": "1.5만원",
          "1.60": "1.5만원",
          "1.67": "1.5만원",
          "1.74": "1.5만원",
        },

        photoRegularPrices: {
          "1.50": "15만원",			
          "1.60": "10만원",
          "1.67": "10만원",
        },
        photoSalePrices: {
		  "1.50": "8만원",
          "1.60": "5만원",
          "1.67": "5만원",
        },

        premiumCoatingRegularPrices: {
          "1.50": "3만원",
          "1.60": "3만원",
          "1.67": "3만원",
          "1.74": "3만원",
        },
        premiumCoatingSalePrices: {
          "1.50": "1만원",
          "1.60": "1만원",
          "1.67": "1만원",
          "1.74": "1만원",
        },
      },

      
    },
  },


국산실속형: {
    manufacturer: "케미",
    desc: "가볍게 사용하는 실속형",
    grade: "합리적 선택",
    badge: "입문 추천",
    variants: {


C0: {
        summary: "부담 없는 실속형버전.",
        goodFor: ["가벼운 근거리 작업", "가격 부담이 큰 고객", "입문형 누진다초점"],
        points: ["실속형", "부담 적은 선택", "기본 기능 중심"],
        image: "/images/ASIO.jpg",
        url: null,

        regularPrices: {
          "1.50": "14만원",
          "1.60": "19.8만원",
          "1.67": "23.8만원",
          "1.74": "29.8만원",
        },
        salePrices: {
          "1.50": "5.9만원",
          "1.60": "7.9만원",
          "1.67": "10.9만원",
          "1.74": "13.9만원",
        },

        tintRegularPrices: {
          "1.50": "3만원",
          "1.60": "3만원",
          "1.67": "3만원",
          "1.74": "3만원",
        },
        tintSalePrices: {
          "1.50": "1.5만원",
          "1.60": "1.5만원",
          "1.67": "1.5만원",
          "1.74": "1.5만원",
        },

        photoRegularPrices: {
          "1.50": "15만원",			
          "1.60": "10만원",
          "1.67": "10만원",
        },
        photoSalePrices: {
		  "1.50": "7만원",
          "1.60": "5만원",
          "1.67": "5만원",
        },

        premiumCoatingRegularPrices: {
          "1.50": "3만원",
          "1.60": "3만원",
          "1.67": "3만원",
          "1.74": "3만원",
        },
        premiumCoatingSalePrices: {
          "1.50": "1만원",
          "1.60": "1만원",
          "1.67": "1만원",
          "1.74": "1만원",
        },
      },



C1: {
        summary: "부담 없는 실속형버전.",
        goodFor: ["가벼운 근거리 작업", "가격 부담이 큰 고객", "입문형 누진다초점"],
        points: ["실속형", "부담 적은 선택", "기본 기능 중심"],
        image: "/images/C1.jpg",
        url: null,

        regularPrices: {
          "1.50": "17.8만원",
          "1.60": "23.8만원",
          "1.67": "29.8만원",
          "1.74": "41.8만원",
        },
        salePrices: {
          "1.50": "8.9만원",
          "1.60": "11.9만원",
          "1.67": "14.9만원",
          "1.74": "20.9만원",
        },

        tintRegularPrices: {
          "1.50": "3만원",
          "1.60": "3만원",
          "1.67": "3만원",
          "1.74": "3만원",
        },
        tintSalePrices: {
          "1.50": "1.5만원",
          "1.60": "1.5만원",
          "1.67": "1.5만원",
          "1.74": "1.5만원",
        },

        photoRegularPrices: {
          "1.50": "15만원",			
          "1.60": "10만원",
          "1.67": "10만원",
        },
        photoSalePrices: {
		  "1.50": "8만원",
          "1.60": "5만원",
          "1.67": "5만원",
        },

        premiumCoatingRegularPrices: {
          "1.50": "3만원",
          "1.60": "3만원",
          "1.67": "3만원",
          "1.74": "3만원",
        },
        premiumCoatingSalePrices: {
          "1.50": "1만원",
          "1.60": "1만원",
          "1.67": "1만원",
          "1.74": "1만원",
        },
      },
	  },
	  },


 E1: {
    manufacturer: "에실로",
    desc: "합리적인 가격의 수입누진렌즈",
    grade: "밸런스형",
    badge: "실속 추천",
    variants: {
      기본: {
        summary:
          "누진다초점 적응이 수월하고 근거리도수가 높지 않은 고객님께 권하는 제품입니다.",
        goodFor: ["가벼운 근거리 작업", "가격 부담이 큰 고객", "예비용 안경"],
        points: ["실속형", "부담 적은 선택", "기본 기능 중심"],
        image: "/images/E12.jpg",
        url: null,

        regularPrices: {
          "1.50": "32만원",
          "1.60": "38만원",
		  "1.67": "48만원",
        },
        salePrices: {
          "1.50": "16만원",
          "1.60": "19만원",
		  "1.67": "24만원",
        },

        tintRegularPrices: {
          "1.50": "3만원",
          "1.60": "3만원",
		  "1.67": "3만원",
        },
        tintSalePrices: {
          "1.50": "1.5만원",
          "1.60": "1.5만원",
		  "1.67": "1.5만원",
        },

        photoRegularPrices: {
          "1.50": "20만원",
          "1.60": "20만원",
          "1.67": "20만원",
        },
        photoSalePrices: {
          "1.50": "10만원",
          "1.60": "10만원",
          "1.67": "10만원",
        },

        premiumCoatingRegularPrices: {
          "1.60": "5만원",
          "1.67": "5만원",
        },
        premiumCoatingSalePrices: {
          "1.60": "2.5만원",
          "1.67": "2.5만원",
        },
      },
    },
  },



  E2: {
    manufacturer: "에실로",
    desc: "디지털 기기 특화 설계",
    grade: "스마트형",
    badge: "실내중심",
    variants: {
      기본: {
        summary: "기본형보다 디지털 디바이스 특화설계로 조금더 편안하게.",
        goodFor: ["디지털기기 특화", "근거리 시야 무난", "가성비좋은 제품"],
        points: ["실내 중심", "근용 특화", "합리적인 가격"],
        image: "/images/E22.jpg",
        url: null,

        regularPrices: {
          "1.50": "39만원",
          "1.60": "47만원",
          "1.67": "55만원",
        },
        salePrices: {
          "1.50": "19.5만원",
          "1.60": "23.5만원",
          "1.67": "27.5만원",
        },

        tintRegularPrices: {
          "1.50": "3만원",
          "1.60": "3만원",
          "1.67": "3만원",
        },
        tintSalePrices: {
          "1.50": "1.5만원",
          "1.60": "1.5만원",
          "1.67": "1.5만원",
        },

        photoRegularPrices: {
          "1.50": "20만원",
          "1.60": "20만원",
          "1.67": "20만원",
        },
        photoSalePrices: {
          "1.50": "10만원",
          "1.60": "10만원",
          "1.67": "10만원",
        },

        premiumCoatingRegularPrices: {
          "1.60": "5만원",
          "1.67": "5만원",
        },
        premiumCoatingSalePrices: {
          "1.60": "2.5만원",
          "1.67": "2.5만원",
        },
      },

      
    },
  },

  E3: {
    manufacturer: "에실로",
    desc: "무난하고 적응 쉬운 선택",
    grade: "밸런스형",
    badge: "적응 추천",
    variants: {
      기본: {
        summary:
          "너무 저가형보다는 편안함을 원하면서도 무난하게 적응하기 좋은 균형형 제품입니다.",
        goodFor: ["원시, 난시고객", "무난한 적응감 선호", "가성비와 편안함 동시 고려"],
        points: ["적응 무난", "균형 잡힌 선택", "합리적인 가성비"],
        image: "/images/E32.jpg",
        url: null,

        regularPrices: {
          "1.50": "48만원",
          "1.60": "57만원",
          "1.67": "71만원",
        },
        salePrices: {
          "1.50": "24만원",
          "1.60": "28.5만원",
          "1.67": "35.5만원",
        },

        tintRegularPrices: {
          "1.50": "3만원",
          "1.60": "3만원",
          "1.67": "3만원",
        },
        tintSalePrices: {
          "1.50": "1.5만원",
          "1.60": "1.5만원",
          "1.67": "1.5만원",
        },

        photoRegularPrices: {
          "1.50": "20만원",
          "1.60": "20만원",
          "1.67": "20만원",
        },
        photoSalePrices: {
          "1.50": "10만원",
          "1.60": "10만원",
          "1.67": "10만원",
        },

        premiumCoatingRegularPrices: {
          "1.60": "5만원",
          "1.67": "5만원",
        },
        premiumCoatingSalePrices: {
          "1.60": "2.5만원",
          "1.67": "2.5만원",
        },
      },
    },
  },

  E4: {
    manufacturer: "에실로",
    desc: "안정적인 시야의 다초점렌즈",
    grade: "밸런스형",
    badge: "적응 추천",
    variants: {
      기본: {
        summary:
          "양안시, 복잡한 도수에서도 안정적인 시야와 빠른 적응을 제공하는 제품",
        goodFor: ["양안 도수차", "빠른 적응감", "합리적인 개인맞춤렌즈"],
        points: ["빠른 적응", "안정적인 선택", "편안한 착용감"],
        image: "/images/E44.jpg",
        url: null,

        regularPrices: {
          "1.50": "62만원",
          "1.60": "75만원",
          "1.67": "86만원",
        },
        salePrices: {
          "1.50": "31만원",
          "1.60": "28.5만원",
          "1.67": "35만원",
        },

        tintRegularPrices: {
          "1.50": "3만원",
          "1.60": "3만원",
          "1.67": "3만원",
        },
        tintSalePrices: {
          "1.50": "1.5만원",
          "1.60": "1.5만원",
          "1.67": "1.5만원",
        },

        photoRegularPrices: {
          "1.50": "20만원",
          "1.60": "20만원",
          "1.67": "20만원",
        },
        photoSalePrices: {
          "1.50": "10만원",
          "1.60": "10만원",
          "1.67": "10만원",
        },

        premiumCoatingRegularPrices: {
          "1.60": "5만원",
          "1.67": "5만원",
        },
        premiumCoatingSalePrices: {
          "1.60": "2.5만원",
          "1.67": "2.5만원",
        },
      },
	  
	  
	  
	  
    },
  },
  
  PB기본형: {
    manufacturer: "에실로",
    desc: "합리적인 가격의 수입누진렌즈",
    grade: "밸런스형",
    badge: "실속 추천",
    variants: {
      기본: {
        summary:
          "누진다초점 적응이 수월하고 근거리도수가 높지 않은 고객님께 권하는 제품입니다.",
        goodFor: ["가벼운 근거리 작업", "가격 부담이 큰 고객", "예비용 안경"],
        points: ["실속형", "부담 적은 선택", "기본 기능 중심"],
        image: "/images/E12.jpg",
        url: null,

        regularPrices: {
          "1.50": "32만원",
          "1.60": "38만원",
		  "1.67": "48만원",
        },
        salePrices: {
          "1.50": "16만원",
          "1.60": "19만원",
		  "1.67": "24만원",
        },

        tintRegularPrices: {
          "1.50": "3만원",
          "1.60": "3만원",
		  "1.67": "3만원",
        },
        tintSalePrices: {
          "1.50": "1.5만원",
          "1.60": "1.5만원",
		  "1.67": "1.5만원",
        },

        photoRegularPrices: {
          "1.50": "20만원",
          "1.60": "20만원",
          "1.67": "20만원",
        },
        photoSalePrices: {
          "1.50": "10만원",
          "1.60": "10만원",
          "1.67": "10만원",
        },

        premiumCoatingRegularPrices: {
          "1.60": "5만원",
          "1.67": "5만원",
        },
        premiumCoatingSalePrices: {
          "1.60": "2.5만원",
          "1.67": "2.5만원",
        },
      },
    },
  },



  PB고급형: {
    manufacturer: "에실로",
    desc: "디지털 기기 특화 설계",
    grade: "스마트형",
    badge: "실내중심",
    variants: {
      기본: {
        summary: "기본형보다 근용 시야 확장으로 조금더 편안하게.",
        goodFor: ["디지털기기 특화", "근거리 시야 무난", "가성비좋은 제품"],
        points: ["실내 중심", "근용 특화", "합리적인 가격"],
        image: "/images/E22.jpg",
        url: null,

        regularPrices: {
          "1.50": "39만원",
          "1.60": "47만원",
          "1.67": "55만원",
        },
        salePrices: {
          "1.50": "19.5만원",
          "1.60": "23.5만원",
          "1.67": "27.5만원",
        },

        tintRegularPrices: {
          "1.50": "3만원",
          "1.60": "3만원",
          "1.67": "3만원",
        },
        tintSalePrices: {
          "1.50": "1만원",
          "1.60": "1만원",
          "1.67": "1만원",
        },

        photoRegularPrices: {
          "1.50": "20만원",
          "1.60": "20만원",
          "1.67": "20만원",
        },
        photoSalePrices: {
          "1.50": "10만원",
          "1.60": "10만원",
          "1.67": "10만원",
        },

        premiumCoatingRegularPrices: {
         
        },
        premiumCoatingSalePrices: {
          
        },
      },

      
    },
  },

  PB프리미엄: {
    manufacturer: "에실로",
    desc: "무난하고 적응 쉬운 선택",
    grade: "밸런스형",
    badge: "적응 추천",
    variants: {
      기본: {
        summary:
          "편안함을 원하면서도 무난하게 적응하기 좋은 균형형 제품입니다.",
        goodFor: ["원시, 난시고객", "무난한 적응감 선호", "가성비와 편안함 동시 고려"],
        points: ["적응 무난", "균형 잡힌 선택", "합리적인 가성비"],
        image: "/images/E32.jpg",
        url: null,

        regularPrices: {
          "1.50": "48만원",
          "1.60": "57만원",
          "1.67": "71만원",
        },
        salePrices: {
          "1.50": "22만원",
          "1.60": "27만원",
          "1.67": "32만원",
        },

        tintRegularPrices: {
          "1.50": "3만원",
          "1.60": "3만원",
          "1.67": "3만원",
        },
        tintSalePrices: {
          "1.50": "1만원",
          "1.60": "1만원",
          "1.67": "1만원",
        },

        photoRegularPrices: {
          "1.50": "20만원",
          "1.60": "20만원",
          "1.67": "20만원",
        },
        photoSalePrices: {
          "1.50": "10만원",
          "1.60": "10만원",
          "1.67": "10만원",
        },

        premiumCoatingRegularPrices: {
         
        },
        premiumCoatingSalePrices: {
          
        },
      },
    },
  },

  PB마스터형: {
    manufacturer: "에실로",
    desc: "안정적인 시야의 다초점렌즈",
    grade: "밸런스형",
    badge: "적응 추천",
    variants: {
      기본: {
        summary:
          "양안시, 복잡한 도수에서도 안정적인 시야와 빠른 적응을 제공하는 제품",
        goodFor: ["양안 도수차", "빠른 적응감", "합리적인 개인맞춤렌즈"],
        points: ["빠른 적응", "안정적인 선택", "편안한 착용감"],
        image: "/images/E44.jpg",
        url: null,

        regularPrices: {
          "1.50": "52만원",
          "1.60": "63만원",
          "1.67": "74만원",
        },
        salePrices: {
          "1.50": "26만원",
          "1.60": "31만원",
          "1.67": "36만원",
        },

        tintRegularPrices: {
          "1.50": "3만원",
          "1.60": "3만원",
          "1.67": "3만원",
        },
        tintSalePrices: {
          "1.50": "1만원",
          "1.60": "1만원",
          "1.67": "1만원",
        },

        photoRegularPrices: {
          "1.50": "20만원",
          "1.60": "20만원",
          "1.67": "20만원",
        },
        photoSalePrices: {
          "1.50": "10만원",
          "1.60": "10만원",
          "1.67": "10만원",
        },

        premiumCoatingRegularPrices: {
          
        },
        premiumCoatingSalePrices: {
          
        },
      },
	  
	  
	  
	  
    },
  },

  컴포트맥스: {
    manufacturer: "바리락스",
    desc: "하루 종일 지속되는 편안함",
    grade: "프리미엄",
    badge: "편안함 추천",
    variants: {
      기본: {
        summary: "전세계 판매1위 브랜드. <바리락스>의 기술력을 확인하세요.",
        goodFor: ["첫 누진으로 빠른적응", "편안한 일상을 원하는 고객", "기존 안경의 자세가 불편한고객"],
        points: ["편안한 자세", "업무 활용도 우수", "프리미엄 입문"],
        image: "/images/BC.jpg",
        url: "https://www.essilor.com/kr-ko/products/varilux/varilux-comfort-max/",

        regularPrices: {
          "1.50": "38만원",
          "1.60": "48만원",
          "1.67": "58만원",
        },
        salePrices: {
          "1.50": "30.4만원",
          "1.60": "38.4만원",
          "1.67": "46.4만원",
        },

        tintRegularPrices: {
          "1.50": "3만원",
          "1.60": "3만원",
          "1.67": "3만원",
        },
        tintSalePrices: {
          "1.50": "2만원",
          "1.60": "2만원",
          "1.67": "2만원",
        },

        photoRegularPrices: {
          "1.50": "20만원",
          "1.60": "20만원",
          "1.67": "20만원",
        },
        photoSalePrices: {
          "1.50": "14만원",
          "1.60": "14만원",
          "1.67": "14만원",
        },

        premiumCoatingRegularPrices: {
          "1.60": "5만원",
          "1.67": "5만원",
        },
        premiumCoatingSalePrices: {
          "1.50": "3.5만원",
          "1.60": "3.5만원",
          "1.67": "3.5만원",
        },
      },

      KAN: {
        summary:
          "전세계 판매1위 브랜드. <바리락스>의 기술력을 확인하세요. ((한국인 전용설계))",
        goodFor: ["첫 누진으로 빠른적응", "편안한 일상을 원하는 고객", "기존 안경의 자세가 불편한고객"],
        points: ["KAN 설계", "적응감 향상", "상위 업그레이드"],
        image: "/images/BC.jpg",
        url: "https://www.essilor.com/kr-ko/products/varilux/varilux-comfort-max/",

        regularPrices: {
          "1.50": "48만원",
          "1.60": "58만원",
          "1.67": "68만원",
        },
        salePrices: {
          "1.50": "33.6만원",
          "1.60": "40.6만원",
          "1.67": "47.6만원",
        },

        tintRegularPrices: {
          "1.50": "3만원",
          "1.60": "3만원",
          "1.67": "3만원",
        },
        tintSalePrices: {
          "1.50": "2만원",
          "1.60": "2만원",
          "1.67": "2만원",
        },

        photoRegularPrices: {
          "1.50": "20만원",
          "1.60": "20만원",
          "1.67": "20만원",
        },
        photoSalePrices: {
          "1.50": "14만원",
          "1.60": "14만원",
          "1.67": "14만원",
        },

        premiumCoatingRegularPrices: {
          "1.60": "5만원",
          "1.67": "5만원",
        },
        premiumCoatingSalePrices: {
          "1.60": "3.5만원",
          "1.67": "3.5만원",
        },
      },
    },
  },

  피지오: {
    manufacturer: "바리락스",
    desc: "다양한 환경에서 깊이 있는 선명함",
    grade: "상위 프리미엄",
    badge: "상위 추천",
    variants: {
      기본: {
        summary: "전세계 판매1위 브랜드. <바리락스>의 기술력을 확인하세요.",
        goodFor: ["장시간 착용 고객", "활동적인 라이프스타일", "원근거리 초점이동이 늦는고객"],
        points: ["실제 동공변화 정밀예측", "장시간 사용 대응", "뛰어난 시각 성능", "야간운전"],
        image: "/images/BP.jpg",
        url: "https://www.essilor.com/kr-ko/products/varilux/varilux-physio-extensee/",

        regularPrices: {
          "1.50": "55만원",
          "1.60": "65만원",
          "1.67": "75만원",
          "1.74": "90만원",
        },
        salePrices: {
          "1.50": "27.5만원",
          "1.60": "32.5만원",
          "1.67": "37.5만원",
          "1.74": "45만원",
        },

        tintRegularPrices: {
          "1.50": "3만원",
          "1.60": "3만원",
          "1.67": "3만원",
        },
        tintSalePrices: {
          "1.50": "2만원",
          "1.60": "2만원",
          "1.67": "2만원",
        },

        photoRegularPrices: {
          "1.50": "20만원",
          "1.60": "20만원",
          "1.67": "20만원",
          "1.74": "20만원",
        },
        photoSalePrices: {
          "1.50": "10만원",
          "1.60": "10만원",
          "1.67": "10만원",
          "1.74": "10만원",
        },

        premiumCoatingRegularPrices: {
          "1.60": "5만원",
          "1.67": "5만원",
        },
        premiumCoatingSalePrices: {
          "1.60": "3.5만원",
          "1.67": "3.5만원",
        },
      },

      Kan: {
        summary:
          "전세계 판매1위 브랜드. <바리락스>의 기술력을 확인하세요. ((한국인 전용설계))",
        goodFor: ["장시간 착용 고객", "활동적인 라이프스타일", "원근거리 초점이동이 늦는고객"],
        points: ["실제 동공변화 정밀예측", "장시간 사용 대응", "뛰어난 시각 성능", "야간운전"],
        image: "/images/BP.jpg",
        url: "https://www.essilor.com/kr-ko/products/varilux/varilux-physio-extensee/",

        regularPrices: {
          "1.50": "65만원",
          "1.60": "75만원",
          "1.67": "85만원",
          "1.74": "100만원",
        },
        salePrices: {
          "1.50": "32.5만원",
          "1.60": "37.5만원",
          "1.67": "42.5만원",
          "1.74": "50만원",
        },

        tintRegularPrices: {
          "1.50": "3만원",
          "1.60": "3만원",
          "1.67": "3만원",
        },
        tintSalePrices: {
          "1.50": "2만원",
          "1.60": "2만원",
          "1.67": "2만원",
        },

        photoRegularPrices: {
          "1.50": "20만원",
          "1.60": "20만원",
          "1.67": "20만원",
          "1.74": "20만원",
        },
        photoSalePrices: {
          "1.50": "10만원",
          "1.60": "10만원",
          "1.67": "10만원",
          "1.74": "10만원",
        },

        premiumCoatingRegularPrices: {
          "1.60": "5만원",
          "1.67": "5만원",
        },
        premiumCoatingSalePrices: {
          "1.60": "3.5만원",
          "1.67": "3.5만원",
        },
      },
    },
  },

  XR: {
    manufacturer: "바리락스",
    desc: "세계최고 다초점렌즈",
    grade: "AI설계 다초점렌즈",
    badge: "모든상황에서 추천",
    variants: {
      기본: {
        summary: "세계판매 1위의 <바리락스>의 최첨단 성능의 누진다초점.",
        goodFor: ["최상위 제품 선호", "적응과 시야 모두 중요", "프리미엄 가치를 정확히 이해하는 고객"],
        points: ["착용 첫날 적응", "빠르고 정확한 시야전환", "AI설계", "언제나 지속되는 선명함"],
        image: "/images/BX.jpg",
        url: "https://www.essilor.com/kr-ko/products/varilux/varilux-xr-series/",

        regularPrices: {
          "1.50": "90만원",
          "1.60": "100만원",
          "1.67": "110만원",
          "1.74": "140만원",
        },
        salePrices: {
          "1.50": "45만원",
          "1.60": "50만원",
          "1.67": "55만원",
          "1.74": "70만원",
        },

        tintRegularPrices: {
          "1.50": "3만원",
          "1.60": "3만원",
          "1.67": "3만원",
        },
        tintSalePrices: {
          "1.50": "2만원",
          "1.60": "2만원",
          "1.67": "2만원",
        },

        photoRegularPrices: {
          "1.50": "20만원",
          "1.60": "20만원",
          "1.67": "20만원",
          "1.74": "20만원",
        },
        photoSalePrices: {
          "1.50": "14만원",
          "1.60": "14만원",
          "1.67": "14만원",
          "1.74": "14만원",
        },

        premiumCoatingRegularPrices: {
          "1.60": "5만원",
          "1.67": "5만원",
        },
        premiumCoatingSalePrices: {
          "1.60": "3.5만원",
          "1.67": "3.5만원",
        },
      },
    },
  },
  
  XRFit: {
    manufacturer: "바리락스",
    desc: "세계최고 다초점렌즈",
    grade: "AI설계 다초점렌즈",
    badge: "모든상황에서 추천",
    variants: {
      기본: {
        summary: "세계판매 1위의 <바리락스>의 최첨단 성능의 누진다초점. 비지오피스로 측정한 모든 개인파라미터 적용하여 완벽한 광학적 성능을 제공합니다.",
        goodFor: ["최상위 제품 선호", "적응과 시야 모두 중요", "프리미엄 가치를 정확히 이해하는 고객"],
        points: ["착용 첫날 적응", "빠르고 정확한 시야전환", "AI설계", "언제나 지속되는 선명함"],
        image: "/images/BX.jpg",
        url: "https://www.essilor.com/kr-ko/products/varilux/varilux-xr-series/",

        regularPrices: {
          "1.50": "120만원",
          "1.60": "135만원",
          "1.67": "150만원",
          "1.74": "180만원",
        },
        salePrices: {
          "1.50": "60만원",
          "1.60": "67.5만원",
          "1.67": "75만원",
          "1.74": "90만원",
        },

        tintRegularPrices: {
          "1.50": "3만원",
          "1.60": "3만원",
          "1.67": "3만원",
        },
        tintSalePrices: {
          "1.50": "2만원",
          "1.60": "2만원",
          "1.67": "2만원",
        },

        photoRegularPrices: {
          "1.50": "20만원",
          "1.60": "20만원",
          "1.67": "20만원",
          "1.74": "20만원",
        },
        photoSalePrices: {
          "1.50": "10만원",
          "1.60": "10만원",
          "1.67": "10만원",
          "1.74": "10만원",
        },

        premiumCoatingRegularPrices: {
          "1.60": "5만원",
          "1.67": "5만원",
        },
        premiumCoatingSalePrices: {
          "1.60": "3.5만원",
          "1.67": "3.5만원",
        },
      },
    },
  },
  
  
  
  
  
  
    로하스: {
    manufacturer: "니콘",
    desc: "첫 누진의 최적의 솔루션",
    grade: "프리미엄",
    badge: "적응감 추천",
    variants: {
      기본: {
        summary: "글로벌 누진브랜드 니콘의 누진 첫 사용자를 위한 최적의 솔루션",
        goodFor: ["첫 누진으로 빠른적응", "편안한 일상을 원하는 고객", "기존 안경의 자세가 불편한고객"],
        points: ["첫 누진 전용", "빠른 일상적응", "프리미엄 입문"],
        image: "/images/BC.jpg",
        url: "https://www.nikonlenswear.com/ko-kr/our-lenses/vision-correction/for-presbyopes/lohas-active",

        regularPrices: {
          "1.50": "40만원",
          "1.60": "50만원",
          "1.67": "60만원",
        },
        salePrices: {
          "1.50": "30만원",
          "1.60": "35만원",
          "1.67": "42만원",
        },

        tintRegularPrices: {
          "1.50": "3만원",
          "1.60": "3만원",
          "1.67": "3만원",
        },
        tintSalePrices: {
          "1.50": "2만원",
          "1.60": "2만원",
          "1.67": "2만원",
        },

        photoRegularPrices: {
          "1.50": "20만원",
          "1.60": "20만원",
          "1.67": "20만원",
        },
        photoSalePrices: {
          "1.50": "14만원",
          "1.60": "14만원",
          "1.67": "14만원",
        },

        premiumCoatingRegularPrices: {
          "1.60": "5만원",
          "1.67": "5만원",
        },
        premiumCoatingSalePrices: {
          
          "1.60": "3.5만원",
          "1.67": "3.5만원",
        },
      },
    },
  },
  
  어드밴스Z: {
    manufacturer: "니콘",
    desc: "디지털 라이프",
    grade: "프리미엄",
    badge: "정적사용자 추천",
    variants: {
      기본: {
        summary: "글로벌 누진브랜드 니콘. 디지털 라이프 스타일의 합리적인 가격의 다초점렌즈",
        goodFor: ["정적사용자", "실내중심 업무용", "디지털기기 사용자"],
        points: ["스마트폰", "실내활동 중심", "프리미엄 입문"],
        image: "/images/BC.jpg",
        url: "https://www.nikonlenswear.com/ko-kr/our-lenses/vision-correction/z-series-progressives/presio-advance-z",

        regularPrices: {
          "1.50": "40만원",
          "1.60": "50만원",
          "1.67": "60만원",
		  "1.74": "75만원",
        },
        salePrices: {
          "1.50": "20만원",
          "1.60": "25만원",
          "1.67": "30만원",
		  "1.74": "37.5만원",
        },

        tintRegularPrices: {
          "1.50": "3만원",
          "1.60": "3만원",
          "1.67": "3만원",
		  "1.74": "3만원",
        },
        tintSalePrices: {
          "1.50": "2만원",
          "1.60": "2만원",
          "1.67": "2만원",
		  "1.74": "2만원",
        },

        photoRegularPrices: {
          "1.50": "20만원",
          "1.60": "20만원",
          "1.67": "20만원",
		  "1.74": "20만원",
        },
        photoSalePrices: {
          "1.50": "10만원",
          "1.60": "10만원",
          "1.67": "10만원",
		  "1.74": "10만원",
        },

        premiumCoatingRegularPrices: {
          "1.60": "5만원",
          "1.67": "5만원",
        },
        premiumCoatingSalePrices: {
          
          "1.60": "3.5만원",
          "1.67": "3.5만원",
        },
      },
    },
  },
  
  
  와이드Z: {
    manufacturer: "니콘",
    desc: "울렁거림 없이 편안하게",
    grade: "프리미엄",
    badge: "편안함 추천",
    variants: {
      기본: {
        summary: "글로벌 누진브랜드 니콘의 베스트셀러 제품. 넓고 탁트인 시야를 경험하세요.",
        goodFor: ["난시가 있는고객", "울렁임에 민감한고객", "단독안경을 운용하는고객"],
        points: ["베스트셀러", "빠른적응속도", "합리적인프리미엄"],
        image: "/images/BP.jpg",
        url: "https://www.nikonlenswear.com/ko-kr/our-lenses/vision-correction/z-series-progressives/presio-wide-z",

        regularPrices: {
          "1.50": "55만원",
          "1.60": "65만원",
          "1.67": "75만원",
		  "1.74": "90만원",
        },
        salePrices: {
          "1.50": "27.5만원",
          "1.60": "32.5만원",
          "1.67": "37.5만원",
		  "1.74": "45만원",
        },

        tintRegularPrices: {
          "1.50": "3만원",
          "1.60": "3만원",
          "1.67": "3만원",
		  "1.74": "3만원",
        },
        tintSalePrices: {
          "1.50": "2만원",
          "1.60": "2만원",
          "1.67": "2만원",
		  "1.74": "2만원",
        },

        photoRegularPrices: {
          "1.50": "20만원",
          "1.60": "20만원",
          "1.67": "20만원",
		  "1.74": "20만원",
        },
        photoSalePrices: {
          "1.50": "10만원",
          "1.60": "10만원",
          "1.67": "10만원",
		  "1.74": "10만원",
        },

        premiumCoatingRegularPrices: {
          "1.60": "5만원",
          "1.67": "5만원",
        },
        premiumCoatingSalePrices: {
          
          "1.60": "3.5만원",
          "1.67": "3.5만원",
        },
      },
    },
  },
  
  파워Z: {
    manufacturer: "니콘",
    desc: "완벽한 균형",
    grade: "프리미엄",
    badge: "최적의 편안함",
    variants: {
      기본: {
        summary: "글로벌 누진브랜드 니콘의 프리미엄. 높은 가입도에서도 편안함을 제공합니다.",
        goodFor: ["가입도가 높은고객", "울렁임에 민감한고객", "단독안경을 운용하는고객"],
        points: ["높은 가입도전용", "빠른적응속도", "최적의 편안함"],
        image: "/images/BP.jpg",
        url: "https://www.nikonlenswear.com/ko-kr/our-lenses/vision-correction/z-series-progressives/presio-power-z",

        regularPrices: {
          "1.50": "70만원",
          "1.60": "85만원",
          "1.67": "100만원",
		  "1.74": "140만원",
        },
        salePrices: {
          "1.50": "32.5만원",
          "1.60": "37.5만원",
          "1.67": "42.5만원",
		  "1.74": "50만원",
        },

        tintRegularPrices: {
          "1.50": "3만원",
          "1.60": "3만원",
          "1.67": "3만원",
		  "1.74": "3만원",
        },
        tintSalePrices: {
          "1.50": "2만원",
          "1.60": "2만원",
          "1.67": "2만원",
		  "1.74": "2만원",
        },

        photoRegularPrices: {
          "1.50": "20만원",
          "1.60": "20만원",
          "1.67": "20만원",
		  "1.74": "20만원",
        },
        photoSalePrices: {
          "1.50": "10만원",
          "1.60": "10만원",
          "1.67": "10만원",
		  "1.74": "10만원",
        },

        premiumCoatingRegularPrices: {
          "1.60": "5만원",
          "1.67": "5만원",
        },
        premiumCoatingSalePrices: {
          
          "1.60": "3.5만원",
          "1.67": "3.5만원",
        },
      },
    },
  },
  
  
  씨맥스Z: {
    manufacturer: "니콘",
    desc: "40억개의 디자인",
    grade: "프리미엄",
    badge: "혁신적인 개인맞춤",
    variants: {
      기본: {
        summary: "글로벌 누진브랜드 니콘의 최고의 누진다초점. 어떠한 조건에서도 선명하고 자연스럽게.",
        goodFor: ["모든 고객", "프리미엄의 가치를 이해하는 고객", "시각적 피로를 줄이고 싶은 고객"],
        points: ["피로감없는 시야", "장시간착용자", "대비감도최적화"],
        image: "/images/BX.jpg",
        url: "https://www.nikonlenswear.com/ko-kr/our-lenses/vision-correction/z-series-progressives/seemax-master-z",

        regularPrices: {
          "1.50": "90만원",
          "1.60": "110만원",
          "1.67": "130만원",
		  "1.74": "160만원",
        },
        salePrices: {
          "1.50": "45만원",
          "1.60": "55만원",
          "1.67": "65만원",
		  "1.74": "80만원",
        },

        tintRegularPrices: {
          "1.50": "3만원",
          "1.60": "3만원",
          "1.67": "3만원",
		  "1.74": "3만원",
        },
        tintSalePrices: {
          "1.50": "2만원",
          "1.60": "2만원",
          "1.67": "2만원",
		  "1.74": "2만원",
        },

        photoRegularPrices: {
          "1.50": "20만원",
          "1.60": "20만원",
          "1.67": "20만원",
		  "1.74": "20만원",
        },
        photoSalePrices: {
          "1.50": "10만원",
          "1.60": "10만원",
          "1.67": "10만원",
		  "1.74": "10만원",
        },

        premiumCoatingRegularPrices: {
          "1.60": "5만원",
          "1.67": "5만원",
        },
        premiumCoatingSalePrices: {
          
          "1.60": "3.5만원",
          "1.67": "3.5만원",
        },
      },
    },
  },
  
  씨맥스얼티밋: {
    manufacturer: "니콘",
    desc: "40억개의 디자인",
    grade: "프리미엄",
    badge: "혁신적인 개인맞춤",
    variants: {
      기본: {
        summary: "글로벌 누진브랜드 니콘의 최고의 누진다초점. 어떠한 조건에서도 선명하고 자연스럽게.",
        goodFor: ["모든 고객", "프리미엄의 가치를 이해하는 고객", "시각적 피로를 줄이고 싶은 고객"],
        points: ["피로감없는 시야", "장시간착용자", "대비감도최적화"],
        image: "/images/BX.jpg",
        url: "https://www.nikonlenswear.com/ko-kr/our-lenses/vision-correction/z-series-progressives/seemax-master-z",

        regularPrices: {
          "1.50": "130만원",
          "1.60": "150만원",
          "1.67": "170만원",
		  "1.74": "210만원",
        },
        salePrices: {
          "1.50": "65만원",
          "1.60": "75만원",
          "1.67": "85만원",
		  "1.74": "105만원",
        },

        tintRegularPrices: {
          "1.50": "3만원",
          "1.60": "3만원",
          "1.67": "3만원",
		  "1.74": "3만원",
        },
        tintSalePrices: {
          "1.50": "2만원",
          "1.60": "2만원",
          "1.67": "2만원",
		  "1.74": "2만원",
        },

        photoRegularPrices: {
          "1.50": "20만원",
          "1.60": "20만원",
          "1.67": "20만원",
		  "1.74": "20만원",
        },
        photoSalePrices: {
          "1.50": "10만원",
          "1.60": "10만원",
          "1.67": "10만원",
		  "1.74": "10만원",
        },

        premiumCoatingRegularPrices: {
          "1.60": "5만원",
          "1.67": "5만원",
        },
        premiumCoatingSalePrices: {
          
          "1.60": "3.5만원",
          "1.67": "3.5만원",
        },
      },
    },
  },
  
  퍼스트A: {
    manufacturer: "펜탁스",
    desc: "디지털 라이프",
    grade: "밸런스형",
    badge: "정적사용자 추천",
    variants: {
      기본: {
        summary: "가성비 수입브랜드. 시선이동을 고려한 맞춤설계",
        goodFor: ["정적사용자", "실내중심 업무용", "입문형 다초점"],
        points: ["개인맞춤", "실내활동 중심", "가성비 입문"],
        image: "/images/E12.jpg",
        url: null,

        regularPrices: {
          "1.50": "27만원",
          "1.60": "31만원",
          "1.67": "38만원",
		  "1.74": "51만원",
        },
        salePrices: {
          "1.50": "18.9만원",
          "1.60": "21.7만원",
          "1.67": "26.6만원",
		  "1.74": "35.7만원",
        },

        tintRegularPrices: {
          "1.50": "3만원",
          "1.60": "3만원",
          "1.67": "3만원",
		  "1.74": "3만원",
        },
        tintSalePrices: {
          "1.50": "2만원",
          "1.60": "2만원",
          "1.67": "2만원",
		  "1.74": "2만원",
        },

        photoRegularPrices: {
          "1.50": "10만원",
          "1.60": "12만원",
          "1.67": "12만원",
		  "1.74": "12만원",
        },
        photoSalePrices: {
          "1.50": "7만원",
          "1.60": "7만원",
          "1.67": "7만원",
		  "1.74": "7만원",
        },

        premiumCoatingRegularPrices: {
          "1.50": "3만원",
          "1.60": "3만원",
          "1.67": "3만원",
		  
        },
        premiumCoatingSalePrices: {
          
          "1.50": "1.6만원",
          "1.60": "1.6만원",
          "1.67": "1.6만원",
		  
        },
      },
    },
  },
  
  
  스마트A: {
    manufacturer: "펜탁스",
    desc: "편안 적응감",
    grade: "밸런스형",
    badge: "균형형",
    variants: {
      기본: {
        summary: "가성비 수입브랜드. 편안한 적응과 균형감 있는 시야",
        goodFor: ["일반 사용자", "올라운드", "무난한 설계"],
        points: ["개인맞춤", "올라운드", "균형있는 시야"],
        image: "/images/E22.jpg",
        url: null,

        regularPrices: {
          "1.50": "42만원",
          "1.60": "46만원",
          "1.67": "52만원",
		  "1.74": "70만원",
        },
        salePrices: {
          "1.50": "27만원",
          "1.60": "30만원",
          "1.67": "36만원",
		  "1.74": "49만원",
        },

        tintRegularPrices: {
          "1.50": "3만원",
          "1.60": "3만원",
          "1.67": "3만원",
		  "1.74": "3만원",
        },
        tintSalePrices: {
          "1.50": "2만원",
          "1.60": "2만원",
          "1.67": "2만원",
		  "1.74": "2만원",
        },

        photoRegularPrices: {
          "1.50": "10만원",
          "1.60": "12만원",
          "1.67": "12만원",
		  "1.74": "12만원",
        },
        photoSalePrices: {
          "1.50": "7만원",
          "1.60": "7만원",
          "1.67": "7만원",
		  "1.74": "7만원",
        },

        premiumCoatingRegularPrices: {
          "1.50": "3만원",
          "1.60": "3만원",
          "1.67": "3만원",
		  
        },
        premiumCoatingSalePrices: {
          
          "1.50": "1.6만원",
          "1.60": "1.6만원",
          "1.67": "1.6만원",
		  
        },
      },
    },
  },
  
   슈퍼A: {
    manufacturer: "펜탁스",
    desc: "양안시",
    grade: "밸런스형",
    badge: "울렁임제거",
    variants: {
      기본: {
        summary: "가성비 수입브랜드. 수차제어로 왜곡과 울렁임제거",
        goodFor: ["양안시설계", "빠른적응", "수차제어"],
        points: ["개인맞춤", "올라운드", "프리미엄 시야"],
        image: "/images/E32.jpg",
        url: null,

        regularPrices: {
          "1.50": "53만원",
          "1.60": "57만원",
          "1.67": "65만원",
		  "1.74": "83만원",
        },
        salePrices: {
          "1.50": "27만원",
          "1.60": "29만원",
          "1.67": "34만원",
		  "1.74": "44만원",
        },

        tintRegularPrices: {
          "1.50": "3만원",
          "1.60": "3만원",
          "1.67": "3만원",
		  "1.74": "3만원",
        },
        tintSalePrices: {
          "1.50": "2만원",
          "1.60": "2만원",
          "1.67": "2만원",
		  "1.74": "2만원",
        },

        photoRegularPrices: {
          "1.50": "10만원",
          "1.60": "12만원",
          "1.67": "12만원",
		  "1.74": "12만원",
        },
        photoSalePrices: {
          "1.50": "7만원",
          "1.60": "7만원",
          "1.67": "7만원",
		  "1.74": "7만원",
        },

        premiumCoatingRegularPrices: {
          "1.50": "3만원",
          "1.60": "3만원",
          "1.67": "3만원",
		  
        },
        premiumCoatingSalePrices: {
          
          "1.50": "1.6만원",
          "1.60": "1.6만원",
          "1.67": "1.6만원",
		  
        },
      },
    },
  },
  
  마스터E: {
    manufacturer: "펜탁스",
    desc: "최고급",
    grade: "밸런스형",
    badge: "프리미엄",
    variants: {
      기본: {
        summary: "가성비 수입브랜드. 펜탁스렌즈의 하이엔드 누진다초점",
        goodFor: ["양안시설계", "빠른적응", "확장된시야"],
        points: ["개인맞춤", "올라운드", "프리미엄 시야"],
        image: "/images/E44.jpg",
        url: null,

        regularPrices: {
          "1.50": "74만원",
          "1.60": "84만원",
          "1.67": "97만원",
		  "1.74": "122만원",
        },
        salePrices: {
          "1.50": "37만원",
          "1.60": "42만원",
          "1.67": "48만원",
		  "1.74": "61만원",
        },

        tintRegularPrices: {
          "1.50": "3만원",
          "1.60": "3만원",
          "1.67": "3만원",
		  "1.74": "3만원",
        },
        tintSalePrices: {
          "1.50": "2만원",
          "1.60": "2만원",
          "1.67": "2만원",
		  "1.74": "2만원",
        },

        photoRegularPrices: {
          "1.50": "10만원",
          "1.60": "12만원",
          "1.67": "12만원",
		  "1.74": "12만원",
        },
        photoSalePrices: {
          "1.50": "7만원",
          "1.60": "7만원",
          "1.67": "7만원",
		  "1.74": "7만원",
        },

        premiumCoatingRegularPrices: {
          "1.50": "3만원",
          "1.60": "3만원",
          "1.67": "3만원",
		  
        },
        premiumCoatingSalePrices: {
          
          "1.50": "1.6만원",
          "1.60": "1.6만원",
          "1.67": "1.6만원",
		  
        },
      },
    },
  },
  
  AM클래식: {
    manufacturer: "호야",
    desc: "합리적 가격과 고품질의 만남",
    grade: "합리성",
    badge: "스마트형",
    variants: {
      기본: {
        summary: "고품질의 호야다초점을 합리적인 가격으로",
        goodFor: ["합리적인 가격"],
        points: ["내면누진설계", "합리적인 가격"],
        image: "/images/E12.jpg",
        url: null,

        regularPrices: {
          "1.50": "19만원",
          "1.60": "29만원",
          "1.67": "39만원",
		  
        },
        salePrices: {
          "1.50": "15만원",
          "1.60": "20만원",
          "1.67": "25만원",
		  
        },

        tintRegularPrices: {
          "1.50": "5만원",
          "1.60": "5만원",
          "1.67": "5만원",
		  
        },
        tintSalePrices: {
          "1.50": "2만원",
          "1.60": "2만원",
          "1.67": "2만원",
		  
        },

        photoRegularPrices: {
          "1.50": "20만원",
          "1.60": "20만원",
          "1.67": "20만원",
		  
        },
        photoSalePrices: {
          "1.50": "10만원",
          "1.60": "10만원",
          "1.67": "10만원",
		  
        },

        premiumCoatingRegularPrices: {
         
		  
        },
        premiumCoatingSalePrices: {
          
          
		  
        },
      },
    },
  },
  
  
  AMM메이리오: {
    manufacturer: "호야",
    desc: "합리적 가격과 고품질의 만남",
    grade: "합리성",
    badge: "스마트형",
    variants: {
      기본: {
        summary: "<15% 확장된 시야>맥스부스터: 원용시야최적화로 편안한 적응 제공",
        goodFor: ["내면누진설계", "빠른적응", "확장된시야"],
        points: ["내면누진설계", "합리적인 가격"],
        image: "/images/E22.jpg",
        url: null,

        regularPrices: {
          "1.50": "25만원",
          "1.60": "35만원",
          "1.67": "45만원",
		  
        },
        salePrices: {
          "1.50": "19만원",
          "1.60": "24만원",
          "1.67": "29만원",
		  
        },

        tintRegularPrices: {
          "1.50": "5만원",
          "1.60": "5만원",
          "1.67": "5만원",
		  
        },
        tintSalePrices: {
          "1.50": "1만원",
          "1.60": "1만원",
          "1.67": "1만원",
		  
        },

        photoRegularPrices: {
          "1.50": "20만원",
          "1.60": "20만원",
          "1.67": "20만원",
		  
        },
        photoSalePrices: {
          "1.50": "10만원",
          "1.60": "10만원",
          "1.67": "10만원",
		  
        },

        premiumCoatingRegularPrices: {
          
		  
        },
        premiumCoatingSalePrices: {
          
          
		  
        },
      },
    },
  },
  
  AMM컨트롤: {
    manufacturer: "호야",
    desc: "합리적 가격과 고품질의 만남",
    grade: "합리성",
    badge: "스마트형",
    variants: {
      기본: {
        summary: "블루라이트 최대 63% 차단 + <15% 확장된 시야>맥스부스터: 원용시야최적화로 편안한 적응 제공",
        goodFor: ["내면누진설계", "빠른적응", "확장된시야"],
        points: ["내면누진설계", "합리적인 가격"],
        image: "/images/E22.jpg",
        url: null,

        regularPrices: {
          "1.50": "30만원",
          "1.60": "40만원",
          "1.67": "50만원",
		  
        },
        salePrices: {
          "1.50": "21만원",
          "1.60": "26만원",
          "1.67": "31만원",
		  
        },

        tintRegularPrices: {
          
		  
        },
        tintSalePrices: {
          
		  
        },

        photoRegularPrices: {
          "1.50": "20만원",
          "1.60": "20만원",
          "1.67": "20만원",
		  
        },
        photoSalePrices: {
          "1.50": "10만원",
          "1.60": "10만원",
          "1.67": "10만원",
		  
        },

        premiumCoatingRegularPrices: {
          
		  
        },
        premiumCoatingSalePrices: {
          
          
		  
        },
      },
    },
  },
  
  다이나믹써미트KR: {
    manufacturer: "호야",
    desc: "자유롭게, 다이나믹한 순간에도 또렷하게",
    grade: "밸런스형",
    badge: "밸런스형",
    variants: {
      기본: {
        summary: "<한국인 맞춤설계> + <퀵포커스:빠르고 정확한 시선전환으로 자연스러운 일상생활>",
        goodFor: ["한국인 전용설계", "첫 누진다초점", "실내외 균형생활"],
        points: ["한국인전용", "적응난이도 감소","부드러운 시선전환"],
        image: "/images/E44.jpg",
        url: null,

        regularPrices: {
          "1.50": "38만원",
          "1.60": "48만원",
          "1.67": "58만원",
		  
        },
        salePrices: {
          "1.50": "25만원",
          "1.60": "30만원",
          "1.67": "35만원",
		  
        },

        tintRegularPrices: {
          "1.50": "5만원",
          "1.60": "5만원",
          "1.67": "5만원",
		  
        },
        tintSalePrices: {
          "1.50": "2.5만원",
          "1.60": "2.5만원",
          "1.67": "2.5만원",
		  
        },

        photoRegularPrices: {
          "1.50": "20만원",
          "1.60": "20만원",
          "1.67": "20만원",
		  
        },
        photoSalePrices: {
          "1.50": "10만원",
          "1.60": "10만원",
          "1.67": "10만원",
		  
        },

        premiumCoatingRegularPrices: {
          "1.50": "5만원",
          "1.60": "5만원",
          "1.67": "5만원",
		  
        },
        premiumCoatingSalePrices: {
          
          "1.50": "1만원",
          "1.60": "1만원",
          "1.67": "1만원",
		  
        },
      },
    },
  },
  
  다이나믹프리미엄KR: {
    manufacturer: "호야",
    desc: "자유롭게, 다이나믹한 순간에도 또렷하게",
    grade: "밸런스형",
    badge: "밸런스형",
    variants: {
      기본: {
        summary: "<한국인 맞춤설계> + <퀵포커스> + <파워옵티마이저> 울렁임을 획기적으로 감소",
        goodFor: ["한국인 전용설계", "첫 누진다초점", "실내외 균형생활"],
        points: ["한국인전용", "적응난이도 감소","부드러운 시선전환"],
        image: "/images/BC.jpg",
        url: "https://www.hoyavision.com/kr/vision-products/progressive-lenses/daynamic/",

        regularPrices: {
          "1.50": "43만원",
          "1.60": "53만원",
          "1.67": "63만원",
		  
        },
        salePrices: {
          "1.50": "25.8만원",
          "1.60": "31.8만원",
          "1.67": "37.8만원",
		  
        },

        tintRegularPrices: {
          "1.50": "5만원",
          "1.60": "5만원",
          "1.67": "5만원",
		  
        },
        tintSalePrices: {
          "1.50": "2.5만원",
          "1.60": "2.5만원",
          "1.67": "2.5만원",
		  
        },

        photoRegularPrices: {
          "1.50": "20만원",
          "1.60": "20만원",
          "1.67": "20만원",
		  
        },
        photoSalePrices: {
          "1.50": "10만원",
          "1.60": "10만원",
          "1.67": "10만원",
		  
        },

        premiumCoatingRegularPrices: {
          "1.50": "5만원",
          "1.60": "5만원",
          "1.67": "5만원",
		  
        },
        premiumCoatingSalePrices: {
          
          "1.50": "2.5만원",
          "1.60": "2.5만원",
          "1.67": "2.5만원",
		  
        },
      },
	  
	  인디비주얼: {
        summary:
          "나만의 시습관과 측정변수를 반영한 개인맞춤 다이나믹 프리미엄",
        goodFor: ["한국인 전용설계", "첫 누진다초점", "실내외 균형생활"],
        points: ["한국인전용", "적응난이도 감소","부드러운 시선전환"],
        image: "/images/BC.jpg",
        url: "https://www.hoyavision.com/kr/vision-products/progressive-lenses/daynamic/",

        regularPrices: {
          "1.50": "48만원",
          "1.60": "58만원",
          "1.67": "68만원",
          
        },
        salePrices: {
          "1.50": "28.8만원",
          "1.60": "34.8만원",
          "1.67": "40.8만원",
          
        },

        tintRegularPrices: {
          "1.50": "5만원",
          "1.60": "5만원",
          "1.67": "5만원",
        },
        tintSalePrices: {
          "1.50": "2.5만원",
          "1.60": "2.5만원",
          "1.67": "2.5만원",
        },

        photoRegularPrices: {
          "1.50": "20만원",
          "1.60": "20만원",
          "1.67": "20만원",
          
        },
        photoSalePrices: {
          "1.50": "10만원",
          "1.60": "10만원",
          "1.67": "10만원",
          
        },

        premiumCoatingRegularPrices: {
		  "1.50": "5만원",
          "1.60": "5만원",
          "1.67": "5만원",
        },
        premiumCoatingSalePrices: {
		  "1.50": "2.5만원",
          "1.60": "2.5만원",
          "1.67": "2.5만원",
        },
      },
	  
	  
    },
  },
  
  발란시스: {
    manufacturer: "호야",
    desc: "시야와 균형감의 완벽한 조화",
    grade: "밸런스형",
    badge: "프리미엄",
    variants: {
      기본: {
        summary: "양면복합설계의 뛰어난 시야와 선명함. 실외활동에 최적화된 렌즈",
        goodFor: ["원용중심 다초점", "뛰어난 원거리시력", "실외활동 라이프스타일"],
        points: ["원용중심", "운전, 아웃도어","와이드한 원거리시야"],
        image: "/images/BC.jpg",
        url: null,

        regularPrices: {
          "1.50": "53만원",
          "1.60": "63만원",
          "1.67": "73만원",
		  "1.74": "88만원",
		  
        },
        salePrices: {
          "1.50": "28만원",
          "1.60": "33만원",
          "1.67": "38만원",
		  "1.74": "46만원",
        },

        tintRegularPrices: {
          "1.50": "5만원",
          "1.60": "5만원",
          "1.67": "5만원",
		  
        },
        tintSalePrices: {
          "1.50": "2.5만원",
          "1.60": "2.5만원",
          "1.67": "2.5만원",
		 
        },

        photoRegularPrices: {
          "1.50": "20만원",
          "1.60": "20만원",
          "1.67": "20만원",
		  
        },
        photoSalePrices: {
          "1.50": "10만원",
          "1.60": "10만원",
          "1.67": "10만원",
		  
        },

        premiumCoatingRegularPrices: {
          "1.50": "5만원",
          "1.60": "5만원",
          "1.67": "5만원",
		  
        },
        premiumCoatingSalePrices: {
          
          "1.50": "1만원",
          "1.60": "1만원",
          "1.67": "1만원",
		  
        },
      },
    },
  },
  
  발란시스KR: {
    manufacturer: "호야",
    desc: "적응이 쉬운 한국인 맞춤 누진다초점",
    grade: "밸런스형",
    badge: "프리미엄",
    variants: {
      기본: {
        summary: "<양면복합설계> 64만명의 한국인Data 반영. 누진다초점의 근본적인 니즈충족. 편안하고 선명한 일상",
        goodFor: ["피로감없는 편안함", "2일이내적응 84%", "제품만족도 95%이상"],
        points: ["2일이내적응 84%", "제품만족도 95%이상"],
        image: "/images/BC.jpg",
        url: "https://www.hoyavision.com/kr/vision-products/progressive-lenses/hoyalux-balansis/",

        regularPrices: {
          "1.50": "58만원",
          "1.60": "68만원",
          "1.67": "78만원",
		  "1.74": "93만원",
		  
        },
        salePrices: {
          "1.50": "31만원",
          "1.60": "36만원",
          "1.67": "41만원",
		  "1.74": "46만원",
        },

        tintRegularPrices: {
          "1.50": "5만원",
          "1.60": "5만원",
          "1.67": "5만원",
		  
        },
        tintSalePrices: {
          "1.50": "2.5만원",
          "1.60": "2.5만원",
          "1.67": "2.5만원",
		  
        },

        photoRegularPrices: {
          "1.50": "20만원",
          "1.60": "20만원",
          "1.67": "20만원",
		  
        },
        photoSalePrices: {
          "1.50": "10만원",
          "1.60": "10만원",
          "1.67": "10만원",
		  
        },

        premiumCoatingRegularPrices: {
          "1.50": "5만원",
          "1.60": "5만원",
          "1.67": "5만원",
		  
        },
        premiumCoatingSalePrices: {
          
          "1.50": "1만원",
          "1.60": "1만원",
          "1.67": "1만원",
		  
        },
      },
	  
	  인디비주얼: {
        summary: "<양면복합설계> 64만명의 한국인Data 반영. 누진다초점의 근본적인 니즈충족. 편안하고 선명한 일상",
        goodFor: ["피로감없는 편안함", "2일이내적응 84%", "제품만족도 95%이상"],
        points: ["2일이내적응 84%", "제품만족도 95%이상"],
        image: "/images/BC.jpg",
        url: "https://www.hoyavision.com/kr/vision-products/progressive-lenses/hoyalux-balansis/",

        regularPrices: {
          "1.50": "65만원",
          "1.60": "75만원",
          "1.67": "85만원",
		  "1.74": "105만원",
		  
        },
        salePrices: {
          "1.50": "32.5만원",
          "1.60": "37.5만원",
          "1.67": "42.5만원",
		  "1.74": "52.5만원",
        },

        tintRegularPrices: {
          "1.50": "5만원",
          "1.60": "5만원",
          "1.67": "5만원",
		  
        },
        tintSalePrices: {
          "1.50": "2.5만원",
          "1.60": "2.5만원",
          "1.67": "2.5만원",
		  
        },

        photoRegularPrices: {
          "1.50": "20만원",
          "1.60": "20만원",
          "1.67": "20만원",
		  
        },
        photoSalePrices: {
          "1.50": "10만원",
          "1.60": "10만원",
          "1.67": "10만원",
		  
        },

        premiumCoatingRegularPrices: {
          "1.50": "5만원",
          "1.60": "5만원",
          "1.67": "5만원",
		  
        },
        premiumCoatingSalePrices: {
          
          "1.50": "2.5만원",
          "1.60": "2.5만원",
          "1.67": "2.5만원",
        },
      },
	  
	  
	  
	  
    },
  },
  
  라이프스타일: {
    manufacturer: "호야",
    desc: "편안하고 선명한 일상생활",
    grade: "밸런스형",
    badge: "프리미엄",
    variants: {
      기본: {
        summary: "손쉽게 적응할 수 있도록 설계된 차세대 LifeStyle 렌즈. 모든것을 고객님께 맞춘 호야의 프리미엄 누진다초점.",
        goodFor: ["피로감없는 편안함", "완벽한 적응 성공률", "눈의피로를 획기적으로 감소"],
        points: ["최상의 시야", "흔들림현상 최소화"],
        image: "/images/BP.jpg",
        url: "https://www.hoyavision.com/kr/vision-products/progressive-lenses/hoyaluxidlifestyle4/",

        regularPrices: {
          "1.50": "75만원",
          "1.60": "85만원",
          "1.67": "95만원",
		  "1.74": "115만원",
		  
        },
        salePrices: {
          "1.50": "37.5만원",
          "1.60": "42.5만원",
          "1.67": "47.5만원",
		  "1.74": "57.5만원",
        },

        tintRegularPrices: {
          "1.50": "5만원",
          "1.60": "5만원",
          "1.67": "5만원",
		  
        },
        tintSalePrices: {
          "1.50": "2.5만원",
          "1.60": "2.5만원",
          "1.67": "2.5만원",
		 
        },

        photoRegularPrices: {
          "1.50": "20만원",
          "1.60": "20만원",
          "1.67": "20만원",
		  
        },
        photoSalePrices: {
          "1.50": "10만원",
          "1.60": "10만원",
          "1.67": "10만원",
		  
        },

        premiumCoatingRegularPrices: {
          "1.50": "5만원",
          "1.60": "5만원",
          "1.67": "5만원",
		  
        },
        premiumCoatingSalePrices: {
          
          "1.50": "2.5만원",
          "1.60": "2.5만원",
          "1.67": "2.5만원",
		  
        },
      },
	  },
  },
  
  MS프로파일: {
    manufacturer: "호야",
    desc: "섬세함의 철학",
    grade: "밸런스형",
    badge: "프리미엄",
    variants: {
      기본: {
        summary: "2020년 옵티션에서 최고의 누진제품으로 선정. 모든 사람의 눈은 지문처럼 서로 다르고, 그에 따라 요구 사항이 다양합니다. 최적화된 안경렌즈를 위한 맞춤화",
        goodFor: ["최고의 누진제품", "선명함과 편안함", "디지털기기 사용시 피로감소"],
        points: ["완벽한 고객맞춤", "적응실패 없음"],
        image: "/images/BX.jpg",
        url: "https://www.hoyavision.com/kr/vision-products/progressive-lenses/hoyalux-mystyle-profile/",

        regularPrices: {
          "1.50": "110만원",
          "1.60": "120만원",
          "1.67": "130만원",
		  "1.74": "150만원",
		  
        },
        salePrices: {
          "1.50": "55만원",
          "1.60": "60만원",
          "1.67": "65만원",
		  "1.74": "75만원",
        },

        tintRegularPrices: {
          "1.50": "5만원",
          "1.60": "5만원",
          "1.67": "5만원",
		  
        },
        tintSalePrices: {
          "1.50": "2.5만원",
          "1.60": "2.5만원",
          "1.67": "2.5만원",
		  
        },

        photoRegularPrices: {
          "1.50": "20만원",
          "1.60": "20만원",
          "1.67": "20만원",
		  
        },
        photoSalePrices: {
          "1.50": "10만원",
          "1.60": "10만원",
          "1.67": "10만원",
		  
        },

        premiumCoatingRegularPrices: {
          
		  
        },
        premiumCoatingSalePrices: {
          
          
		  
        },
      },
	  },
  },
  
  
  
  
  
  
};
