        ...
        {sampleLenses.map((lens, idx) => {
          ...
          return (
            <div
			  ...
			  ${isE3 ? 'ring-4 ring-gray-300/80' : ''}
			  ${isE4 ? 'ring-4 ring-pink-300/80' : ''}`}
              style={{ zIndex: isE3 || isE4 ? 5 : 1 }}
            >
			
              {isE3 && (
                <div className="absolute left-6 -top-4 bg-gray-400 text-white font-extrabold px-5 py-1 rounded-full shadow-xl text-sm tracking-wide border-2 border-gray-600 animate-bounce whitespace-nowrap z-20">
                  프리미엄 고객 선택 1위
                </div>
              )}
